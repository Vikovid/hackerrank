/*
    ...
    Comparison Sorting:
    Quicksort usually has aa running time of "n*log(n)".
    But is there an algorithm than can sort even faster? In general
    this is not possible. Most sorting algorithms are comparison sorts, i.e
    they sort a list just by comparing the elements to one another. 

    A comparision sort algorithm cannot beat n*log(n) (worst case) 
    running time, since n*log(n) represent the minimum number of 
    comparisions needed to know where o place each element. 
    ...
    occurs in the original array, you increment the counter at that
    index. At the end, run through your counting array, printing the value
...
function countingSort(arr){
    ...
    for(let i = 0; i < result.length; i++)
        result[numbers[i]] = arr.filter(n => n === numbers[i]).length;
